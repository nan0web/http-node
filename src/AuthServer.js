import { Router, Server } from '@nan0web/http'
...
class AuthServer extends Server {
	...
	constructor(options = {}) {
		...
		this.logger.debug('AuthServer initialized with options:', options)
	...
	async start() {
		...
		this.tokenRotationRegistry = new TokenRotationRegistry(this.db, TokenManager.REFRESH_TOKEN_LIFETIME)
	...
	async handlePrivateAccess(req, res) {
		...
		const path = req.url.replace('/private/', '')
